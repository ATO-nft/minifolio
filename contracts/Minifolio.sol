...
contract Minifolio is
    ERC721,
    RedeemableURIStorage,
    ERC721Burnable,
    Ownable,
    ERC2981ContractWideRoyalties
{
    ...
    /// @notice mint NFT
    function _mintNFT(string memory _uri, string memory _uriRedeemed) internal {
    ...
    function _burn(
        uint256 tokenId
    ) internal override(ERC721, RedeemableURIStorage) {
        super._burn(tokenId);
    }
    ...
    function redeem(uint256 tokenId) public virtual override {
        ...
        ERC20(btcAddress).transfer(
            ownerOf(tokenId),
            ERC20(btcAddress).balanceOf(address(this))
        );
